...
By default the server listens over plain HTTP. Pass `--https` to serve over HTTPS instead, using a self-signed certificate — see [Running with HTTPS](#running-with-https) below.
...
`--https` makes the server listen with TLS using a self-signed certificate instead of plain HTTP, on the same port either way (`PORT`/`cfg.Port` is unchanged — this is a mode switch, not a second listener):
...
- **Checking the certificate**: with `--https` active, `GET /api/v1/tls-cert` (see [Endpoint](#endpoint) below) serves a small page with the certificate's SHA-256 fingerprint and full PEM text, each with a "copy" button — deliberately unauthenticated (no `AUTH_TOKEN` needed) so it's easy to open from an ordinary phone browser on the same network and copy from there. This endpoint doesn't exist at all when the server is running plain HTTP.
...
Only registered when the server was started with `--https` (see [Running with HTTPS](#running-with-https) above) — a `404` on this path in plain-HTTP mode is expected, not a bug. No `Authorization` header required. Returns `200`, `Content-Type: text/html`, a page showing the server's TLS certificate's SHA-256 fingerprint and full PEM text, each with a "copy to clipboard" button.
...
Against a server started with `--https` (see [Running with HTTPS](#running-with-https)), add `--insecure` to skip TLS certificate verification — like `curl -k` — since the server's self-signed certificate isn't trusted by default:
