...
A Go REST service that decides and serves what an e-ink screen connected to an ESP32 should display. The ESP32 will poll the endpoint hourly; the endpoint renders today's agenda from a single reference Google Calendar followed by the current shopping list from a single reference Google Sheet (both via the same one-time-authorized OAuth credential, refreshed unattended — see "Calendar credentials" below), and in future iterations will also vary by time of day. Runs autonomously on a VPS. This is the user's first Go project — prioritize idiomatic, explainable code over shortcuts.
...
- **`cmd/calendarcheck`** / **`cmd/sheetscheck`**: small permanent diagnostic CLIs, one per integration, that dump raw API data as JSON, bypassing `internal/calendar`'s `Row` / `internal/shoppinglist`'s parsing on purpose (they exist to inspect the real shape Google sends, e.g. what a genuinely blank sheet row looks like, or debugging unexpected calendar data).
- **Shopping-list error isolation**: in `internal/server/handlers.go`, a calendar fetch failure still replaces the whole screen with an error message (unchanged from iteration 2, since the agenda is the primary content) — but a shopping-list fetch failure only replaces that section's lines (`shoppingListLines` returns a one-line Spanish error string on `err != nil`) so the agenda stays visible. This is a deliberate asymmetry, not an oversight: the two content sources aren't treated as equally critical.
